...
data_neg <- read_xlsx("data/05/05_SuppresNeg.xlsx", sheet = "neg")

# read NAPS data and rename ----------------------------------------------------
rti_neg <- read_xlsx("data/05/05_RtiNeg.xlsx") %>% 
  rename(rtime = RT,
         rindex = RTI)

# split data -------------------------------------------------------------------
meta_data_neg <- data_neg %>% 
  select(Name, `m/z`, RT)

feature_data_neg <- data_neg %>% 
  select(-`m/z`, -RT)

# calculate the mean of all injections and suppression/enhancing factor ---------
feature_data_sum <- feature_data_neg %>% 
  ...
  t() %>% 
  as_tibble() %>% 
  mutate(Name = meta_data_neg$Name)
...
feature_data_suppres <- feature_data_sum %>% 
  ...
  full_join(meta_data_neg, by = "Name") %>% 
  select(Name, `m/z`, RT,
         suppres_Cel_1dil20,
         suppres_Cel_1dil40,
         suppres_Cel_1dil80,
...
feature_data_suppres <- feature_data_suppres %>% 
  mutate(rti = indexRtime(RT, rti_neg))

# get data around NAPS ---------------------------------------------------------
suppres_data <- tibble()

for(i in 1:nrow(rti_neg)) {
  
  filter_rtime <- rti_neg$rtime[i]
  filter_rindex <- rti_neg$rindex[i]
  
  suppres_filter <- feature_data_suppres %>% 
    filter(abs(RT - filter_rtime) < 0.2) %>% 
...
suppres_data %>% 
  ...
  scale_x_continuous(limits = c(-0.20, 0.205)) +
